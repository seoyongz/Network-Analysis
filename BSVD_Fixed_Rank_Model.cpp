...
arma::cx_vec roots_quartic(dcomplex a3, dcomplex a2, dcomplex a1, dcomplex a0) {
  // Find roots of quartic equation
...
///////////////////////////////////////////////////////////////////////
// [[Rcpp::export]]
Rcpp::List BSVD_fixed(arma::mat Y, arma::mat U, arma::mat V, arma::mat D, const int nrank,
  ...
  for(t=0; t<niter; t++){
    ...
    for(j=0; j<Ycol; j++){    // j-th column updatae with gibbs sampler
      ...
      if(Dmat(j, j) != 0){          // d_j 가 0이 아닐때
        // nrank += 1;
        
        // // Setting U[, -j] and V[, -j], D[-j, -j]
        for(k=0; k<j; k++){             // column index
        ...
        for(i=0; i<n0; i++){
          for(k=0; k<Yrow; k++){ 
            v_param(i) += NvE_j(i, k)*Umat(k, j); 
          }
        }
        v_param = phi*Dmat(j, j)*v_param;
        ...
        for(l=0; l<Ycol; l++){
          Vmat(l, j) = 0.0;
          for(k=0; k<n0; k++){ 
            Vmat(l, j) += Nv(l, k)*vj(k); 
          }
        }
        
        
        // Sample uj from vMF
        arma::mat NuE_j(n0, Yrow, fill::zeros);
        for(i=0; i<m0; i++){
          for(l=0; l<Ycol; l++){
            for(k=0; k<Yrow; k++){
        ...
        for(i=0; i<m0; i++){
          for(k=0; k<Ycol; k++){ 
            u_param(i) += NuE_j(i, k)*Vmat(k, j); 
          }
        }
        u_param = phi*Dmat(j, j)*u_param;
        ...
        for(i=0; i<Ycol; i++){
          for(k=0; k<Yrow; k++){ 
            UjE_j(i) += Umat(k, j)*E_j(k, i); 
          }
        }
        double UjE_jVj = 0.0;
        for(k=0; k<Ycol; k++){ 
          UjE_jVj += UjE_j(k)*Vmat(k, j); 
        }
        
        d_mu = (UjE_jVj*phi + mu*psi)/(phi + psi);
        d_sd = std::sqrt(1.0/(phi + psi));
        
        // Sampling d(j)
        Dmat(j, j) = R::rnorm(d_mu, d_sd);
    ...
    mu_post_mean = (psi*sum_dj + mu_pr_mean*mu_pr_precision)/(psi*(nrank*1.0) + mu_pr_precision);
    mu_post_sd = sqrt(1.0/(psi*(nrank*1.0) + mu_pr_precision));
    ...
    if(t % nprint == 0) {
      Rprintf("iter - %d | ", t);
      Rprintf("phi %f mu %f psi %f phi_post_b %f psi_post_b %f \n", phi, mu, psi, phi_post_b, psi_post_b);
    }
